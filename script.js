...
class CommentsSystem {
    ...
    submitComment() {
        ...
        const comment = {
            ...
            author: userName.value.trim() || '匿名用户',
    ...
    renderComments() {
        ...
        const commentsHtml = this.comments.length > 0 
            ? `<h4>留言列表：</h4>${this.comments.map(comment => this.renderComment(comment)).join('')}`
            : `<h4>留言列表：</h4><div class="no-comments">暂无留言，快来抢沙发吧！</div>`;
    ...
    loadComments() {
        try {
            const saved = localStorage.getItem('neuroamateurs_comments');
    ...
    saveComments() {
        try {
            localStorage.setItem('neuroamateurs_comments', JSON.stringify(this.comments));
        } catch (e) {
